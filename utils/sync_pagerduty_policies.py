#!/usr/bin/env python
#
# Synchronize list of escalation policies with notification channels in Diginfra
#
import argparse
import copy
import json
...
from dicclient import DiMonitorClient

#
# Parse arguments
#
parser = argparse.ArgumentParser(description='Synchronize PagerDuty escalation policies with Diginfra, '
                                             'to make sure each escalation policy has a notification '
                                             'channel enabled in Diginfra')
parser.add_argument('diginfra-token', nargs=1, help='Diginfra API token')
...
parser.add_argument(
    '--link',
    action='store_true',
    help='Set to creat notification channels in Diginfra and services in PagerDuty for all escalation policies'
)
parser.add_argument(
    '--unlink',
    action='store_true',
...
def run(diginfra_token, pager_duty_id, pager_duty_token, link, unlink, dry_run):
    if not link and not unlink:
        # by default, you're going to link accounts
        link = True

    diginfra = DiMonitorClient(diginfra_token)
    pager_duty = PagerDutyAPI(pager_duty_token)
    actions_factory = ActionFactory(diginfra, pager_duty, pager_duty_id)

    #
    # Get list of Diginfra notification channels
    #
    ok, res = diginfra.list_notification_channels()
    if not ok:
        print('\nUnable to fetch Diginfra notification channels')
        print(res)
        sys.exit(1)
    ...
    print('Found {} PagerDuty notification {} configured in Diginfra'.format(
    ...
    #
    # Get Diginfra vendor configuration
    #
    diginfra_vendor = pager_duty.get('/vendors', {'query': 'diginfra', 'limit': 1,
    ...
    #
    # Find integrations with Diginfra
    #
    service_integration_keys = {}
    for service in services:
        service['diginfra_integrations'] = [integration for integration in service['integrations']
                                          if
                                          'vendor' in integration and integration['vendor'] and integration['vendor'][
                                              'id'] == diginfra_vendor['id']]

        for integration in service['diginfra_integrations']:
    ...
    if unlink:
        #
        # delete all PagerDuty notification channels in Diginfra
        #
        for channel in pager_duty_channels:
            actions.append({
                'info': 'Diginfra: Delete channel "{}" ({})'.format(channel['name'], channel['id']),
                'fn': actions_factory.delete_notification_channel(channel)
            })

        #
        # delete integration with Diginfra
        #
        for service in services:
            if service['diginfra_integrations']:
                if len(service['diginfra_integrations']) == len(service['integrations']):
                    #
                    # service connected to Diginfra only: delete service
                    #
                    actions.append({
                    ...
                    #
                    # service with some integrations with Diginfra: delete individual integrations
                    #
                    for integration in service['diginfra_integrations']:
                        actions.append(
                            {
    ...
    if link:
        #
        # delete all PagerDuty notification channels in Diginfra that do NOT have an integration in PagerDuty
        #
        for channel in pager_duty_channels:
        ...
        for policy in escalation_policies:
            service_name = '{} (Diginfra)'.format(policy['name'])

            policy_services = [service for service in services if service['escalation_policy']['id'] == policy['id']]
            diginfra_services = [service for service in policy_services if service['diginfra_integrations']]
            disconnected_services = []
            for service in diginfra_services:
                for integration in service['integrations']:
                    if integration['vendor'] and \
                            integration['vendor']['id'] == diginfra_vendor['id'] and \
            ...
            if not diginfra_services:
                #
                # create service and integration in PagerDuty, and notification channel in Diginfra
                #
                actions.append({'info': 'Create service, integration, and notification channel for policy "{}"'.format(
                    policy['name']), 'fn': actions_factory.create_all(policy, diginfra_vendor)})
    ...
    if actions:
        ...
        if unlink:
            print('All escalation policies have been disconnected from Diginfra!')
        if link:
            print('All escalation policies are already connected to Diginfra!')
...
class ActionFactory():
    def __init__(self, diginfra, pager_duty, pager_duty_id):
        self._diginfra = diginfra
    ...
    def delete_notification_channel(self, channel):
        def fn():
            self._diginfra.delete_notification_channel(channel)

        return fn

    def create_all(self, policy, diginfra_vendor):
        def fn():
            new_service = self._pager_duty.post('/services', {
                'service': {
                    'type': 'service',
                    'name': '{} (Diginfra)'.format(policy['name']),
            ...
            self._diginfra.create_notification_channel({
                'type': 'PAGER_DUTY',
                'enabled': True,
    ...
    def rename_notification_channel(self, channel, channel_name, service_name):
        def fn():
            ...
            self._diginfra.update_notification_channel(new_channel)
...
run(args['diginfra-token'][0], args['pagerduty-account-id'][0],
